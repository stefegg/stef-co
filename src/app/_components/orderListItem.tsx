...
export default function OrderListItem(props: OrderListItemProps) {
  ...
  return (
    <div
      className={`w-full border-2 border-${appTheme}-text rounded-lg  hover:bg-${appTheme}-containerBg hover:border-${appTheme}-secondary shadow-[0px_0px_3px_3px_rgba(0,0,0,0.3)]`}
    >
      <Link
        className="flex flex-row justify-between w-full h-full py-4 px-2"
        href={`my-orders/${order.id}`}
      >
        <div className="w-1/3 pl-4">{`Placed On: ${format(
          order.createdAt,
          "MM/dd/yyyy"
        )}`}</div>
